...
function SidebarMenu({ className, ...props }: React.ComponentProps<"ul">) {
  return (
    <ul
      ...
      data-sidebar="menu"
      className={cn("flex w-full min-w-0 flex-col gap-25", className)}
      {...props}
    />
  );
}
...
const sidebarMenuButtonVariants = cva(
    ...
    "[&>span:last-child]:truncate [&>svg]:size-6 [&>svg]:shrink-0",
  {
    variants: {
      ...
      activate: {
        true: "font-medium text-sidebar-accent-foreground [&>svg]:bg-[hsl(var(--sidebar-accent-2))]  [&>svg]:rounded-md ",
      },
    },
    defaultVariants: {
      variant: "default",
...
function SidebarMenuButton({
  ...
  const button = (
    <Comp
      ...
      className={cn(
        sidebarMenuButtonVariants({ variant, size, activate: isActive }),
        className
      )}
      {...props}
    />
